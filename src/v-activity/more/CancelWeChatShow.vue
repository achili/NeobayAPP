<template>
  <div class="cancel-weChat-show" @click.stop="closeOwn">
    <div class="popup animated bounceInUp">
      <div class="title">{{$t('message.SureCancelWeChat')}}?</div>
      <div class="content">
        <div class="select" >
          <div class="text" @click.stop="selectYes">{{$t('message.Yes')}}</div>
        </div>
        <div class="select" >
    ...
    methods: {
      ...
      selectYes() {
        ...
        this.user.httpCancelAssociation(this.user.unionid, function () {
          ...
          that.$store.state.plus.nativeUI.toast(that.$t('message.CancelAssociationSuccess'),{duration:"short"});
          that.closeCall();
        })
      },
      selectNo() {

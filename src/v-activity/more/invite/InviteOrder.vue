<template>
  <div class="order-enroll-container">
    <div class="order-top">
      ...
      <div class="page-title">{{$t('message.MyInvite')}}</div>
    ...
    <div class="order-list-item">
      ...
      >
        <div class="order-list-item-container">
          <div class="list-item " v-for="(item,index) in pager.inviteVisitors" data-type="0" :key="index">
            <div class="list-box"  @click="skip">
              <div class="order-item-top">
                <div class="order-item-top-title">
                  {{$t('message.InviteName')}}: {{item.name}}
                </div>
                <div class="order-item-top-edit" >
                  {{item.invitingTime | yearTime}}
                </div>
              </div>
              ...
              <div class="room-item-footer">
                <div class="room-order-footer-day">
                  {{$t('message.InviteDate')}}: {{item.invitingTime | hourTime}}-{{item.accessTime | hourTime}}
                </div>
                <div class="room-order-footer-time" @click.stop="orderEdit(item)">
                  {{$t('message.Edit')}}
                </div>
              </div>
            </div>
  ...
  export default{
    data () {
      return {
        ...
        pager:this.$store.state.InviteVisitorPager,
        title:"message.Invite",
        name:"activeOrder",
    ...
    methods : {
      orderEdit(inviteVisitor) {
        ...
        this.$store.state.temps.inviteClose = function () {
          that.refresh()
        }
        this.$router.openActivity("invite_apply","slide-left",{inviteId:inviteVisitor.id});
      },
      closeAllPopup() {
